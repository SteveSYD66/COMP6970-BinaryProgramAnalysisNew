...
class BasicBlock_CFG:
    ...
    def split_basic_block(self, bb, jump_to_instr):
        ...
        print()
        print("This basic block below is being deleted!")
        print(f"Basic Block {hex(bb.start_instr.address)}: {hex(bb.start_instr.address)} - {hex(bb.end_instr.address)}")
        print(f"\tInstructions: {[hex(instr.address) for instr in bb.bb_instr_list]}")
        ...
        print()
        print(f"Block 1 below created after splitting basic block {hex(bb.start_instr.address)}!")
        print(f"Basic Block {hex(bb_1.start_instr.address)}: {hex(bb_1.start_instr.address)} - {hex(bb_1.end_instr.address)}")
        print(f"\tInstructions: {[hex(instr.address) for instr in bb_1.bb_instr_list]}")
        print(f"\tInstructions: {[instr.mnemonic for instr in bb_1.bb_instr_list]}")
        print(f"\tNext Instructions: {[hex(instr.address) for instr in bb_1.bb_next_instrs]}")
        self.add_basic_block(bb_2)
        print()
        print(f"Block 2 below created after splitting basic block {hex(bb.start_instr.address)}!")
        print(f"Basic Block {hex(bb_2.start_instr.address)}: {hex(bb_2.start_instr.address)} - {hex(bb_2.end_instr.address)}")
        print(f"\tInstructions: {[hex(instr.address) for instr in bb_2.bb_instr_list]}")
        print(f"\tInstructions: {[instr.mnemonic for instr in bb_2.bb_instr_list]}")
        print(f"\tNext Instructions: {[hex(instr.address) for instr in bb_2.bb_next_instrs]}")
            ...
            if instr is not None:
                ...
                if (mnemonic.startswith("j") or mnemonic.startswith("call")):
                    ...
                    if jump_addr is not None:
                        string = "0x%04x (%04d): (%02d) %-4s\t%s (%04d)" % (instr.address,
                                                                            instr.address,
                                                                            instr.size,
                                                                            instr.mnemonic,
                                                                            instr.op_str,
                        ...
                        string = "0x%04x (%04d): (%02d) %-4s\t%s" % (instr.address,
                                                            instr.address,
                                                            instr.size,
                                                            instr.mnemonic,
                                                            instr.op_str)
    ...
    def create_basic_blocks(self, instr_list):
        ...
        while addr < len(self.idx_instr_list): # or self.next_instr_addr_list[0] is not None:
            ...
            if (mnemonic.startswith("j") or 
                ...
                if addr == next_instr_addr:
                    ...
                    if self.basic_block_already_exists(self.basic_blocks, next_instr):
                        if self.next_instr_addr_list:
                            ...
                            print()
                            print(f"Next instr addr list is not empty! Next address is : {hex(addr)}" )
                        else:
                ...
                print()
                print(f"Basic Block {hex(current_bb.start_instr.address)}: {hex(current_bb.start_instr.address)} - {hex(current_bb.end_instr.address)}")
                print(f"\tInstructions: {[hex(instr.address) for instr in current_bb.bb_instr_list]}")
                print(f"\tInstructions: {[instr.mnemonic for instr in current_bb.bb_instr_list]}")
                print(f"\tNext Instructions: {[hex(instr.address) for instr in current_bb.bb_next_instrs]}")
