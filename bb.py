            ...
            if instr is not None:
                ...
                if (mnemonic.startswith("j") or mnemonic.startswith("call")):
                    ...
                    if jump_addr is not None:
                        string = "0x%04x (%04d): (%02d) %-4s\t%s (%04d)" % (
                        instr.address, instr.address, instr.size, instr.mnemonic, instr.op_str, jump_addr)
                        f.write(string + "\n")
                    else:
                        string = "0x%04x (%04d): (%02d) %-4s\t%s" % (
                        instr.address, instr.address, instr.size, instr.mnemonic, instr.op_str)
                        f.write(string + "\n")
                else:
                    string = "0x%04x (%04d): (%02d) %-4s\t%s" % (
                    instr.address, instr.address, instr.size, instr.mnemonic, instr.op_str)
